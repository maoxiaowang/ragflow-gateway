...
from app.api.v1.ragflow.schemas import Dataset, Document, UploadDocument
...
@router.post("/datasets/{dataset_id}/documents", response_model=Response[List[UploadDocument]])
async def upload_document(
        dataset_id: str,
        files: List[UploadFile] = File(...),
):
    docs, count = await service.upload_document(dataset_id, files=files)
    return Response(data=[UploadDocument(**doc) for doc in docs])
